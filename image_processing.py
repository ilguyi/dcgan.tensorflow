...
def inputs(batch_size=None, num_preprocess_threads=None):
...
def distorted_inputs(batch_size=None, num_preprocess_threads=None):
...
#                       tf.expand_dims(distorted_image, 0), max_outputs=10)
...
def batch_inputs(batch_size, train, num_preprocess_threads=None,
                 num_readers=1):
  ...
  with tf.name_scope('batch_processing'):
    ...
    tf_record_pattern = os.path.join(dataset_dir, 'train-*')
    ...
    # 1 image uses 64*64*3*4 bytes = 48kB
    # The default input_queue_memory_factor is 16 implying a shuffling queue
    # size: examples_per_shard * 16 * 48kB = 96MB
